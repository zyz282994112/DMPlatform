...
public class FieldFilterSetCondition extends HttpServlet implements Servlet {
	public FieldFilterSetCondition() {
	...
	protected void doPost(HttpServletRequest request,
		...
		else
		{
			response.getWriter().write("<script>alert(\"fields information not defined!\");" +
					"window.location.href='FieldFilterSelectDs.do'</script>");
			return;
		}
		if(id > 0)
		{
		...
		response.getWriter().write("<script>window.location.href='view/dataset/fieldFilter.jsp'</script>");
		return;
		}
		else
		{
